...
#define connectActionToThis(action, slot)                                      \
    connect(action, &QAction::triggered, this,                                 \
            &std::remove_pointer_t<decltype(this)>::slot)
...
#define lazyconnect(src, signal, dest, slot)                                   \
    connect(src, &std::remove_reference_t<decltype(*src)>::signal, dest,       \
            &std::remove_reference_t<decltype(*dest)>::slot)
...
#define connectLambda(src, signal, dest, lambda)                               \
    connect(src, &std::remove_reference_t<decltype(*src)>::signal, dest, lambda)
