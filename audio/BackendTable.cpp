...
struct SoundIoDevice* BackendTable::getDevice(SoundIoBackend backendType, const char *id) noexcept {
...
struct SoundIoDevice* BackendTable::getDefaultDevice() noexcept {
    ...
    for (unsigned i = 0; i != mBackendCount; ++i) {
        ...
        if (soundio->current_backend != SoundIoBackendNone) {
            if (!table.isEmpty()) {
                struct SoundIoDevice *device = soundio_get_output_device(
                            soundio, table.defaultDevice());
                return device;
            }
        }
        ++backendIter;
    }
    return nullptr; // this SHOULD never happen, worst case the dummy device gets chosen
}
